...
import '../style.css'
import ModalChangeTable from '@/app/Modal/ChangeTable'

const HomeID = ({ params }: { params: { id: string } }) => {
  ...
  return (
    <>
      {pathname == `/home/${params.id}` ? <Menu keys={"1"} /> : null}
        ...
        <main style={{ display: "flex", flexDirection: "column", padding: '3rem', minHeight: '100vh', marginLeft: "30px" }}>
          <div style={{ display: "flex", flexDirection: "row" }}>
            ...
            <div style={{ textAlign: "center", width: 350, maxWidth: "100%", borderRadius: 15, padding: 25, backgroundColor: "#FFFFFF", marginLeft: 10 }}>
              ...
              <Divider />
              {/* {Array.isArray(PurchaseList) ? PurchaseList.map((item) => (item.order.map((element: any, index: number) => {
                return (
